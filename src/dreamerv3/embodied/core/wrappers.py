...
# MOD: Alternative to ResizeImage below. Commented here: https://github.com/danijar/dreamerv3/issues/12#issuecomment-1913642884
import cv2
import gym
class CustomResizeImage(gym.ObservationWrapper):
    # Adapts obs_space to specified shape. Supports resizing, flattening (2D/3D > 1D), and channel addition (2D > 3D). Usage examples:
    # env = gym.make('ChessVsSelf-v1')
    # env = CustomResizeImage(env, shape=(84, 84), grayscale=False)
    # env = CustomResizeImage(env, shape=(64, 64, 1), grayscale=True)  # To resize to grayscale image
    # env = CustomResizeImage(env, shape=(8*8,), grayscale=False)  # To flatten the observation space
    def __init__(self, env, shape, repeat_channels=3):
      super().__init__(env)
      self.shape = shape
      self.grayscale = repeat_channels == 1
      self.is_flatten = isinstance(shape, (int, tuple)) and len(shape) == 1
      self.is_resize = isinstance(shape, tuple)
        
      if self.is_flatten:
        new_shape = (np.prod(env.observation_space.shape),)
      elif self.is_resize:
        if len(shape) == 2:
    ...
    def observation(self, observation):
      if self.is_flatten:
        ...
        resized_obs = cv2.resize(observation, self.shape[:2], interpolation=cv2.INTER_AREA)
        if len(self.shape) == 2:
          if self.grayscale:
            resized_obs = cv2.cvtColor(resized_obs, cv2.COLOR_RGB2GRAY)
            resized_obs = np.expand_dims(resized_obs, axis=-1)
          else:
            resized_obs = cv2.cvtColor(resized_obs, cv2.COLOR_RGB2BGR)
            resized_obs = np.repeat(resized_obs[:, :, np.newaxis], 3, axis=2)
        return resized_obs
      return observation
#   def observation(self, observation):
#     isWrongFormat = isinstance(observation, tuple) and len(observation) > 0 and isinstance(observation[0], np.ndarray)
#     if isWrongFormat: observation = observation[0]  # Extract the image part, if observation is a tuple w/ an ndarray as 1st element
#     return cv2.resize(observation, self.shape[::-1], interpolation=cv2.INTER_AREA) # Resize the observation

  

# MOD: Enhance logs/renders during training & eval.
import matplotlib.pyplot as plt
import cv2
def print_step_info(args, tran):
  # Export the action space (keypresses, clicks...) as the AI plays in real time.
  if getattr(args, 'export_actions', False):
    # action = tran['action']
    reward, is_last, is_terminal = tran['reward'], tran['is_last'], tran['is_terminal']
    timelog = f"{time.strftime('%H:%M:%S', time.localtime())}"
    print(f"👉 Reward: {reward}   —   Done: {is_last} / {is_terminal}   —   {timelog}")
    # TODO: log the action to an external file / db

  # If `env.render()` won't work (like in Crafter), use this to visualize the AI playing in real time.
  if getattr(args, 'plt_render', False):
    if args.plt_render == 'matplotlib':
      plt.imshow(tran['image'])
      plt.show(block=False)
      plt.pause(0.001)  # Pause to ensure the plot updates
      plt.clf()  # Clear the plot so that the next image replaces this one
    if args.plt_render == 'opencv':
      cv2.imshow('Frame', tran['image'])
      cv2.waitKey(1)  # Wait for a key press for a short while (1 ms)
